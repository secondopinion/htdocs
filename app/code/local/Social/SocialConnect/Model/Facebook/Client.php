...
class Social_SocialConnect_Model_Facebook_Client
{
    const REDIRECT_URI_ROUTE = 'socialconnect/facebook/connect';

    const XML_PATH_ENABLED = 'customer/social_socialconnect_facebook/enabled';
    const XML_PATH_CLIENT_ID = 'customer/social_socialconnect_facebook/client_id';
    const XML_PATH_CLIENT_SECRET = 'customer/social_socialconnect_facebook/client_secret';
    ...
    protected function fetchAccessToken()
    {
        if(empty($_REQUEST['code'])) {
            throw new Exception(
                Mage::helper('social_socialconnect')
        ...
        if($response->isError()) {
            ...
            if(($status == 400 || $status == 401)) {
                if(isset($decoded_response->error->message)) {
                    ...
                    $message = Mage::helper('social_socialconnect')
                        ->__('Unspecified OAuth error occurred.');
                }

                throw new Social_SocialConnect_FacebookOAuthException($message);
            } else {
                $message = sprintf(
                    Mage::helper('social_socialconnect')
...
class Social_SocialConnect_FacebookOAuthException extends Exception
{}